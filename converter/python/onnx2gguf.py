...
from core.ir import GraphIR, ModelIR, NodeIR, TensorIR, ValueInfoIR
...
def export_ir_json_from_model(
    ...
    output_file.write_text(json.dumps(model_ir.to_dict(), ensure_ascii=False, indent=indent), encoding="utf-8")
    return output_file
...
def resolve_output_paths(input_model_path: str | Path, output_dir: str | Path | None = None) -> tuple[Path, Path]:
    ...
    graph_path = base_dir / f"{stem}_graph.json"
    weights_path = base_dir / f"{stem}_weights.gguf"
    return graph_path, weights_path


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Convert ONNX model to mini2gguf graph JSON + GGUF weights")
