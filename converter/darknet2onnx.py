# download from https://raw.githubusercontent.com/jkjung-avt/tensorrt_demos/master/yolo/yolo_to_onnx.py
# original name is: yolo_to_onnx.py
#
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Convert Darknet YOLO cfg+weights to ONNX'
    )
    parser.add_argument(
        '-c', '-cfg', '--cfg', type=str, required=True,
        help='path to darknet .cfg file')
    parser.add_argument(
        '-w', '-weights', '--weights', type=str, required=True,
        help='path to darknet .weights file')
    parser.add_argument(
        '-o', '-output-path', '--output-path', type=str, default=None,
        help='path to output .onnx file '
             '(default: <cfg_dir>/<cfg_stem>.onnx)')
...
def extract_yolo_metadata(cfg_file_path, output_tensor_names=None):
    with open(cfg_file_path, 'r') as f:
        lines = f.readlines()
    ...
    for line in lines:
        ...
        if stripped.startswith('[') and stripped.endswith(']'):
            if in_yolo:
                ...
                current = {}
            in_yolo = (stripped == '[yolo]')
            continue
        if not in_yolo or '=' not in stripped:
            continue

        key, value = [part.strip() for part in stripped.split('=', 1)]
        if key in ('anchors', 'mask'):
            current[key] = _parse_num_list(value)
        elif key in ('classes', 'num', 'random'):
            current[key] = int(float(value))
        elif key in ('jitter', 'ignore_thresh', 'truth_thresh'):
    ...
    for idx, block in enumerate(blocks):
        ...
        if isinstance(mask, list) and isinstance(anchors, list):
            selected = []
            for m in mask:
                m = int(m)
                if m * 2 + 1 < len(anchors):
                    selected.extend([anchors[m * 2], anchors[m * 2 + 1]])
            head['selected_anchors'] = selected
        if output_tensor_names is not None and idx < len(output_tensor_names):
            head['output'] = output_tensor_names[idx]
        heads.append(head)
...
def set_model_metadata_kv(model_def, key, value):
    value_str = str(value)
    for entry in model_def.metadata_props:
        if entry.key == key:
            entry.value = value_str
            return
    entry = model_def.metadata_props.add()
    entry.key = key
    entry.value = value_str
...
def main():
    ...
    if output_file_path is None:
        cfg_abs = os.path.abspath(cfg_file_path)
        cfg_dir = os.path.dirname(cfg_abs)
        cfg_stem = os.path.splitext(os.path.basename(cfg_abs))[0]
        output_file_path = os.path.join(cfg_dir, cfg_stem + '.onnx')
        print('output path not specified, use default: %s' % output_file_path)
    model_name = os.path.splitext(os.path.basename(output_file_path))[0]
    ...
    yolo_metadata = extract_yolo_metadata(cfg_file_path, output_tensor_names)

    print('Checking ONNX model...')
    onnx.checker.check_model(yolo_model_def)

    set_model_metadata_kv(
        yolo_model_def,
        'mini2gguf.darknet.yolo',
        json.dumps(yolo_metadata, separators=(',', ':')))
    set_model_metadata_kv(yolo_model_def, 'model.family', 'yolo')
    set_model_metadata_kv(yolo_model_def, 'model.version', '3')
    set_model_metadata_kv(yolo_model_def, 'model.class', 'detection')
