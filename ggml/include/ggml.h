...
extern "C" {
    ...
    // gather values from `a` using per-element indices from `indices` along `axis`
    // output type follows `a`, output shape follows `indices`
    GGML_API struct ggml_tensor * ggml_gather_elements(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
    ...
    // Depthwise 2D convolution + channel bias fused in backend writeback
    // bias supports [C] or [1, 1, C, 1]
    GGML_API struct ggml_tensor * ggml_conv_2d_dw_direct_bias(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
            struct ggml_tensor  * b,
    ...
    // 2D convolution + channel bias fused in backend writeback
    // bias supports [OC] or [1, 1, OC, 1]
