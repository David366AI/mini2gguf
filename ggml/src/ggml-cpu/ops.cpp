...
template <typename T>
static inline float ggml_reduce_max_to_f32(T v) {
    return v;
}

template <>
inline float ggml_reduce_max_to_f32<ggml_fp16_t>(ggml_fp16_t v) {
    return ggml_fp16_to_fp32(v);
}

template <typename T>
static inline T ggml_reduce_max_from_f32(float v) {
    return (T) v;
}

template <>
inline ggml_fp16_t ggml_reduce_max_from_f32<ggml_fp16_t>(float v) {
    return ggml_fp32_to_fp16(v);
}

template <typename T>
static void ggml_compute_forward_reduce_max_t(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    for (int64_t i = 0; i < out_total; ++i) {
        ...
        for (int64_t r = 0; r < reduce_ne; ++r) {
            ...
            const char * src_ptr = (const char *) src0->data
                ...
                + coord[2] * src0->nb[2]
                + coord[3] * src0->nb[3];
            row_max = std::max(row_max, ggml_reduce_max_to_f32(*(const T *) src_ptr));
        }

        *(T *) dst_base = ggml_reduce_max_from_f32<T>(row_max);
    }
}

void ggml_compute_forward_reduce_max(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_reduce_max_t<float>(params, dst);
            } break;
        case GGML_TYPE_F16:
            {
                ggml_compute_forward_reduce_max_t<ggml_fp16_t>(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_call_mul_mat_layout(
        ggml_type type,
        const ggml_compute_params * params,
        int64_t dst_ne0,
        int64_t dst_ne1,
...
static void ggml_call_mul_mat_layout_mixed(
        ggml_type src0_type,
        ggml_type src1_type,
        const ggml_compute_params * params,
        int64_t dst_ne0,
        ...
        size_t dst_nb1,
        size_t src1_nb1) {
    const ggml_type_traits * traits0 = ggml_get_type_traits(src0_type);
    const ggml_type_traits * traits1 = ggml_get_type_traits(src1_type);
    ...
    src0.ne[2] = 1;
    src0.ne[3] = 1;
    src0.nb[0] = traits0->type_size;
    src0.nb[1] = k * traits0->type_size;
    src0.nb[2] = src0.nb[1];
    src0.nb[3] = src0.nb[2];
    src0.data  = (void *) src0_data;
...
static void ggml_call_mul_mat(ggml_type type, const ggml_compute_params * params, int64_t m, int64_t n, int64_t k,
    ...
    // Legacy layout used by conv2d path:
    // dst shape [n, m], src0 rows=n (kernel), src1 cols=m (patches), contiguous row-major output.
    ggml_call_mul_mat_layout(type, params, n, m, k, b, a, c, n * sizeof(float));
}

static void ggml_call_mul_mat_mixed(
        ggml_type src0_type,
        ggml_type src1_type,
        const ggml_compute_params * params,
        int64_t m,
    ...
    // Legacy layout used by conv2d path:
    // dst shape [n, m], src0 rows=n (kernel), src1 cols=m (patches), contiguous row-major output.
    ggml_call_mul_mat_layout_mixed(src0_type, src1_type, params, n, m, k, b, a, c, n * sizeof(float), a_nb1);
}
...
enum ggml_conv_bias_layout {
    GGML_CONV_BIAS_NONE = 0,
    GGML_CONV_BIAS_1D,
    GGML_CONV_BIAS_4D,
};

enum ggml_conv2d_algo {
    GGML_CONV2D_ALGO_GENERIC = 0,
    GGML_CONV2D_ALGO_POINTWISE_1X1,
    GGML_CONV2D_ALGO_DIRECT_3X3,
};

enum ggml_conv2d_dw_algo {
    GGML_CONV2D_DW_ALGO_GENERIC = 0,
    GGML_CONV2D_DW_ALGO_DIRECT_3X3,
};

static inline bool ggml_cpu_disable_conv_special() {
    static const bool disabled = []() {
        const char * v = std::getenv("GGML_CPU_DISABLE_CONV_SPECIAL");
        if (v == nullptr) {
            return false;
        }
        return v[0] != '\0' && v[0] != '0';
...
static inline ggml_conv2d_algo ggml_conv2d_select_algo(
        int64_t knl_w,
        int64_t knl_h,
        int32_t stride_x,
        int32_t stride_y,
        ...
        int32_t dilation_x,
        int32_t dilation_y) {
    if (ggml_cpu_disable_conv_special()) {
        return GGML_CONV2D_ALGO_GENERIC;
    }
    ...
    if (knl_w == 1 && knl_h == 1 &&
        ...
        return GGML_CONV2D_ALGO_POINTWISE_1X1;
    }

    if (knl_w == 3 && knl_h == 3 &&
        (stride_x == 1 || stride_x == 2) &&
        stride_x == stride_y &&
        pad_x == 1 && pad_y == 1 &&
        dilation_x == 1 && dilation_y == 1) {
        return GGML_CONV2D_ALGO_DIRECT_3X3;
    }

    return GGML_CONV2D_ALGO_GENERIC;
}

static inline ggml_conv_bias_layout ggml_conv_bias_detect_layout(
        const ggml_tensor * bias,
        int64_t channels,
        int64_t batch) {
    if (bias == nullptr) {
        return GGML_CONV_BIAS_NONE;
    }

    const int n_dims = ggml_n_dims(bias);
    if (n_dims == 1 && bias->ne[0] == channels) {
        return GGML_CONV_BIAS_1D;
    }
    if (n_dims == 4 &&
        bias->ne[0] == 1 &&
        bias->ne[1] == 1 &&
        bias->ne[2] == channels &&
        (bias->ne[3] == 1 || bias->ne[3] == batch)) {
        return GGML_CONV_BIAS_4D;
    }

    GGML_ABORT("unsupported conv bias layout");
    return GGML_CONV_BIAS_NONE;
}

static inline float ggml_conv_bias_get_value(
        const ggml_tensor * bias,
        ggml_conv_bias_layout layout,
        int64_t channel_idx,
        int64_t batch_idx) {
    if (layout == GGML_CONV_BIAS_NONE) {
        return 0.0f;
    }

    const char * base = (const char *) bias->data;
    const char * ptr = nullptr;

    if (layout == GGML_CONV_BIAS_1D) {
        ptr = base + channel_idx * bias->nb[0];
    } else {
        const int64_t bias_batch = bias->ne[3] == 1 ? 0 : batch_idx;
        ptr = base + channel_idx * bias->nb[2] + bias_batch * bias->nb[3];
    }
    ...
    switch (bias->type) {
        ...
        default:
            GGML_ABORT("unsupported conv bias type");
            return 0.0f;
    }
}

static inline ggml_conv2d_dw_algo ggml_conv2d_dw_select_algo(
        int64_t knl_w,
        int64_t knl_h,
        int32_t dilation_x,
        int32_t dilation_y) {
    if (ggml_cpu_disable_conv_special()) {
        return GGML_CONV2D_DW_ALGO_GENERIC;
    }

    if (knl_w == 3 && knl_h == 3 && dilation_x == 1 && dilation_y == 1) {
        return GGML_CONV2D_DW_ALGO_DIRECT_3X3;
    }
...
static void ggml_compute_forward_conv_2d_impl(const ggml_compute_params * params,
    ...
    const ggml_conv_bias_layout bias_layout = ggml_conv_bias_detect_layout(bias, c_out, dst->ne[3]);
    const bool has_bias = bias_layout != GGML_CONV_BIAS_NONE;
    ...
    const bool use_direct_writeback =
        ggml_is_contiguous(dst) &&
        dst->type == GGML_TYPE_F32 &&
        dst->ne[3] == 1;
    const ggml_conv2d_algo conv_algo = ggml_conv2d_select_algo(
        knl_w, knl_h, stride_x, stride_y, pad_x, pad_y, dilation_x, dilation_y);

    const bool use_pointwise_whcn_nopack =
        conv_algo == GGML_CONV2D_ALGO_POINTWISE_1X1 &&
    ...
    if (use_pointwise_whcn_nopack) {
        ...
        for (int64_t oc = oc_start; oc < oc_end; ++oc) {
            ...
            if (bias_val == 0.0f) {
                memset(dst_plane, 0, (size_t) patch_total * sizeof(float));
            } else {
                for (int64_t p = 0; p < patch_total; ++p) {
                    dst_plane[p] = bias_val;
                }
            }

            if (kernel_type == GGML_TYPE_F32) {
                const float * k_row = (const float *) knl_data + oc * c_in;
                for (int64_t ic = 0; ic < c_in; ++ic) {
                    const float * src_plane = (const float *) ((const char *) src->data + ic * src->nb[2]);
                    ggml_vec_mad_f32((int) patch_total, dst_plane, src_plane, k_row[ic]);
                }
            } else {
                const ggml_fp16_t * k_row = (const ggml_fp16_t *) knl_data + oc * c_in;
                for (int64_t ic = 0; ic < c_in; ++ic) {
                    const float * src_plane = (const float *) ((const char *) src->data + ic * src->nb[2]);
                    const float w = GGML_CPU_FP16_TO_FP32(k_row[ic]);
                    ggml_vec_mad_f32((int) patch_total, dst_plane, src_plane, w);
                }
            }
        }
    ...
    const size_t src_patch_stride = (size_t) c_in * src_type_size;
    const size_t src_plane_stride = (size_t) dst_w * src_patch_stride;
    const size_t src_batch_stride = (size_t) dst_h * src_plane_stride;
    const bool src_type_compatible =
        src->type == kernel_type || src->type == GGML_TYPE_F32;
    const bool src_is_dense_cwhn =
        src->nb[2] == src_type_size &&
        src->nb[0] == src_patch_stride &&
        src->nb[1] == src_plane_stride &&
        src->nb[3] == src_batch_stride;
    const bool use_pointwise_no_pack =
        conv_algo == GGML_CONV2D_ALGO_POINTWISE_1X1 &&
        src_type_compatible &&
        src_is_dense_cwhn;

    int64_t patches_per_batch = 0;
    int64_t batch_n = 0;
    if (use_pointwise_no_pack && use_direct_writeback) {
        patches_per_batch = patch_total;
        batch_n = patch_total > 0 ? 1 : 0;
    } else {
        const int64_t packed_patch_bytes = use_pointwise_no_pack ? 0 : knl_n * traits->type_size;
        const int64_t space_per_patch = packed_patch_bytes + (use_direct_writeback ? 0 : c_out * (int64_t) sizeof(float));
        const int64_t batch_size = params->wsize / space_per_patch;
        patches_per_batch = batch_size > 8 ? (batch_size / 8) * 8 : batch_size;
        GGML_ASSERT(patches_per_batch > 0 && batch_size >= 1);
        batch_n = (patch_total + patches_per_batch - 1) / patches_per_batch;
    }
    ...
    for (int64_t batch_i = 0; batch_i < batch_n; ++batch_i) {
        ...
        // For the pointwise path with dense CWHN input, consume input directly
        // as GEMM rows and skip explicit patch packing.
        if (!use_pointwise_no_pack && conv_algo == GGML_CONV2D_ALGO_POINTWISE_1X1) {
            for (int64_t p = patch_start; p < patch_end; ++p) {
                const int64_t batch_idx = p / (dst_w * dst_h);
                const int64_t pos       = p % (dst_w * dst_h);
                const int64_t src_y     = pos / dst_w;
                const int64_t src_x     = pos % dst_w;

                const char * src_base = (const char *) src->data +
                    batch_idx * src->nb[3] +
                    src_x * src->nb[0] +
                    src_y * src->nb[1];
                ...
                if (kernel_type == GGML_TYPE_F32) {
                    float * dst_row_f32 = (float *) dst_row;
                    if (!src_is_f16 && src->nb[2] == sizeof(float)) {
                        memcpy(dst_row_f32, src_base, c_in * sizeof(float));
                    } else {
                        for (int64_t ic = 0; ic < c_in; ++ic) {
                            const char * src_ptr = src_base + ic * src->nb[2];
                            dst_row_f32[ic] = src_is_f16
                                ? GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) src_ptr)
                                : *(const float *) src_ptr;
                        }
                    }
                } else {
                    ggml_fp16_t * dst_row_f16 = (ggml_fp16_t *) dst_row;
                    if (src_is_f16 && src->nb[2] == sizeof(ggml_fp16_t)) {
                        memcpy(dst_row_f16, src_base, c_in * sizeof(ggml_fp16_t));
                    } else {
                        for (int64_t ic = 0; ic < c_in; ++ic) {
                            const char * src_ptr = src_base + ic * src->nb[2];
                            const float src_val = src_is_f16
                                ? GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) src_ptr)
                                : *(const float *) src_ptr;
                            dst_row_f16[ic] = GGML_CPU_FP32_TO_FP16(src_val);
                        }
                    }
                }
            } // patches handled by this thread
        } else if (!use_pointwise_no_pack && conv_algo == GGML_CONV2D_ALGO_DIRECT_3X3) {
            const int64_t stride = stride_x;
            for (int64_t p = patch_start; p < patch_end; ++p) {
                const int64_t batch_idx = p / (dst_w * dst_h);
                const int64_t pos       = p % (dst_w * dst_h);
                const int64_t out_y     = pos / dst_w;
                const int64_t out_x     = pos % dst_w;

                const int64_t center_y = out_y * stride;
                const int64_t center_x = out_x * stride;
                const bool is_interior =
                    center_y > 0 && center_y < src_h - 1 &&
                    center_x > 0 && center_x < src_w - 1;

                const char * src_base = (const char *) src->data + batch_idx * src->nb[3];
                char * dst_row = (char *) tmp + (p % patches_per_batch) * knl_n * traits->type_size;

                if (kernel_type == GGML_TYPE_F32) {
                    float * dst_row_f32 = (float *) dst_row;
                    for (int64_t ic = 0; ic < c_in; ++ic) {
                        float * dst_ic = dst_row_f32 + ic * 9;
                        if (is_interior) {
                            const char * src_c = src_base + ic * src->nb[2];
                            const char * p00 = src_c + (center_x - 1) * src->nb[0] + (center_y - 1) * src->nb[1];
                            const char * p01 = p00 + src->nb[0];
                            const char * p02 = p01 + src->nb[0];
                            const char * p10 = p00 + src->nb[1];
                            const char * p11 = p10 + src->nb[0];
                            const char * p12 = p11 + src->nb[0];
                            const char * p20 = p10 + src->nb[1];
                            const char * p21 = p20 + src->nb[0];
                            const char * p22 = p21 + src->nb[0];

                            if (src_is_f16) {
                                dst_ic[0] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p00);
                                dst_ic[1] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p01);
                                dst_ic[2] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p02);
                                dst_ic[3] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p10);
                                dst_ic[4] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p11);
                                dst_ic[5] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p12);
                                dst_ic[6] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p20);
                                dst_ic[7] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p21);
                                dst_ic[8] = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) p22);
                            } else {
                                dst_ic[0] = *(const float *) p00;
                                dst_ic[1] = *(const float *) p01;
                                dst_ic[2] = *(const float *) p02;
                                dst_ic[3] = *(const float *) p10;
                                dst_ic[4] = *(const float *) p11;
                                dst_ic[5] = *(const float *) p12;
                                dst_ic[6] = *(const float *) p20;
                                dst_ic[7] = *(const float *) p21;
                                dst_ic[8] = *(const float *) p22;
                            }
                        } else {
                            for (int64_t ky = 0; ky < 3; ++ky) {
                                const int64_t sy = center_y + ky - 1;
                                for (int64_t kx = 0; kx < 3; ++kx) {
                                    const int64_t sx = center_x + kx - 1;
                                    const int64_t dst_idx = ky * 3 + kx;
                                    float src_val = 0.0f;
                                    if (sy >= 0 && sy < src_h && sx >= 0 && sx < src_w) {
                                        const char * src_ptr = src_base + sx * src->nb[0] + sy * src->nb[1] + ic * src->nb[2];
                                        src_val = src_is_f16
                                            ? GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) src_ptr)
                                            : *(const float *) src_ptr;
                                    }
                                    dst_ic[dst_idx] = src_val;
                                }
                            }
                        }
                    }
                    ...
                    ggml_fp16_t * dst_row_f16 = (ggml_fp16_t *) dst_row;
                    for (int64_t ic = 0; ic < c_in; ++ic) {
                        ggml_fp16_t * dst_ic = dst_row_f16 + ic * 9;
                        if (is_interior) {
                            const char * src_c = src_base + ic * src->nb[2];
                            const char * p00 = src_c + (center_x - 1) * src->nb[0] + (center_y - 1) * src->nb[1];
                            const char * p01 = p00 + src->nb[0];
                            ...
                            if (src_is_f16) {
                                dst_ic[0] = *(const ggml_fp16_t *) p00;
                                dst_ic[1] = *(const ggml_fp16_t *) p01;
                                dst_ic[2] = *(const ggml_fp16_t *) p02;
                                dst_ic[3] = *(const ggml_fp16_t *) p10;
                                dst_ic[4] = *(const ggml_fp16_t *) p11;
                                dst_ic[5] = *(const ggml_fp16_t *) p12;
                                dst_ic[6] = *(const ggml_fp16_t *) p20;
                                dst_ic[7] = *(const ggml_fp16_t *) p21;
                                dst_ic[8] = *(const ggml_fp16_t *) p22;
                            } else {
                                dst_ic[0] = GGML_CPU_FP32_TO_FP16(*(const float *) p00);
                                dst_ic[1] = GGML_CPU_FP32_TO_FP16(*(const float *) p01);
                                dst_ic[2] = GGML_CPU_FP32_TO_FP16(*(const float *) p02);
                                dst_ic[3] = GGML_CPU_FP32_TO_FP16(*(const float *) p10);
                                dst_ic[4] = GGML_CPU_FP32_TO_FP16(*(const float *) p11);
                                dst_ic[5] = GGML_CPU_FP32_TO_FP16(*(const float *) p12);
                                dst_ic[6] = GGML_CPU_FP32_TO_FP16(*(const float *) p20);
                                dst_ic[7] = GGML_CPU_FP32_TO_FP16(*(const float *) p21);
                                dst_ic[8] = GGML_CPU_FP32_TO_FP16(*(const float *) p22);
                            }
                        } else {
                            for (int64_t ky = 0; ky < 3; ++ky) {
                                const int64_t sy = center_y + ky - 1;
                                for (int64_t kx = 0; kx < 3; ++kx) {
                                    ...
                                    dst_ic[dst_idx] = GGML_CPU_FP32_TO_FP16(src_val);
                                }
                            }
                        }
                    }
                    ...
                    for (int64_t ky = 0; ky < knl_h; ++ky) {
                        for (int64_t kx = 0; kx < knl_w; ++kx) {
                            ...
                            if (sy < 0 || sy >= src_h || sx < 0 || sx >= src_w) {
                                ...
                                const char * src_ptr = src_base + sx * src->nb[0] + sy * src->nb[1] + ic * src->nb[2];
                                if (src_is_f16) {
                                    src_val = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *) src_ptr);
                                } else {
                                    src_val = *(const float *) src_ptr;
                                }
                            }
        ...
        if (use_pointwise_no_pack) {
            ...
            if (use_direct_writeback) {
                ...
                gemm_output = (float *) tmp;
                GGML_ASSERT(gemm_output + patch_n * c_out <= (float *) ((char *) tmp + params->wsize));
                ggml_call_mul_mat_mixed(
                    kernel_type,
                    src->type,
                    params,
                    patch_n,
        ...
        if (use_direct_writeback) {
            if (has_bias) {
                const int64_t bias_per_thread = (patch_n + params->nth - 1) / params->nth;
                const int64_t bias_start = params->ith * bias_per_thread;
                const int64_t bias_end = std::min(bias_start + bias_per_thread, patch_n);

                for (int64_t i = bias_start; i < bias_end; ++i) {
...
struct ggml_conv_2d_dw_params {
    ...
    int dilation_y;
    const ggml_tensor * bias;
    ggml_conv_bias_layout bias_layout;
    ggml_conv2d_dw_algo algo;
};
...
static void ggml_compute_forward_conv_2d_dw_cwhn(
...
#endif
    const bool use_3x3_fast = p.algo == GGML_CONV2D_DW_ALGO_DIRECT_3X3;
    ...
    for (int64_t row = row_start; row < row_end; ++row) {
        ...
        for (int64_t dst_x = 0; dst_x < p.dst_w; ++dst_x) {
            ...
            if (use_3x3_fast) {
                const bool is_interior =
                    src_y_base >= 0 && src_y_base + 2 < p.src_h &&
                    src_x_base >= 0 && src_x_base + 2 < p.src_w;

                if (is_interior) {
                    const int64_t idx00 = (src_y_base       * p.src_w + src_x_base)      * c;
                    const int64_t idx01 = idx00 + c;
                    const int64_t idx02 = idx01 + c;
                    const int64_t idx10 = idx00 + p.src_w * c;
                    const int64_t idx11 = idx10 + c;
                    const int64_t idx12 = idx11 + c;
                    const int64_t idx20 = idx10 + p.src_w * c;
                    const int64_t idx21 = idx20 + c;
                    const int64_t idx22 = idx21 + c;

#ifdef GGML_SIMD
                    for (int64_t c_i = 0; c_i < c_pkg_end; c_i += pkg_size) {
                        GGML_F32_VEC sum = GGML_F32_VEC_ZERO;

                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 0 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx00 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 1 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx01 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 2 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx02 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 3 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx10 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 4 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx11 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 5 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx12 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 6 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx20 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 7 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx21 + c_i));
                        sum = GGML_F32_VEC_FMA(sum, GGML_F32_VEC_LOAD(knl_data + 8 * c + c_i), GGML_F32_VEC_LOAD(src_data + idx22 + c_i));
                    ...
                    for (int64_t c_i = c_pkg_end; c_i < c; ++c_i) {
                        float sum = 0.0f;
                        sum += knl_data[0 * c + c_i] * src_data[idx00 + c_i];
                        sum += knl_data[1 * c + c_i] * src_data[idx01 + c_i];
                        sum += knl_data[2 * c + c_i] * src_data[idx02 + c_i];
                        sum += knl_data[3 * c + c_i] * src_data[idx10 + c_i];
                        sum += knl_data[4 * c + c_i] * src_data[idx11 + c_i];
                        sum += knl_data[5 * c + c_i] * src_data[idx12 + c_i];
                        sum += knl_data[6 * c + c_i] * src_data[idx20 + c_i];
                        sum += knl_data[7 * c + c_i] * src_data[idx21 + c_i];
                        sum += knl_data[8 * c + c_i] * src_data[idx22 + c_i];
                        dst_data[c_i] = sum;
                    }

                    if (p.bias_layout != GGML_CONV_BIAS_NONE) {
                        for (int64_t c_i = 0; c_i < c; ++c_i) {
                            dst_data[c_i] += ggml_conv_bias_get_value(p.bias, p.bias_layout, c_i, batch_idx);
                        }
                    }
                    continue;
                }
...
static void ggml_compute_forward_conv_2d_dw_whcn(
    ...
    for (int64_t i = start; i < end; ++i) {
        ...
        const float bias_val = p.bias_layout != GGML_CONV_BIAS_NONE
            ? ggml_conv_bias_get_value(p.bias, p.bias_layout, channel_idx, batch_idx)
            : 0.0f;
        const float * knl_data = (const float *)kernel->data + channel_idx * p.knl_w * p.knl_h;
            ...
            for (int64_t dst_x = 0; dst_x < p.dst_w; ++dst_x) {
                ...
                if (use_3x3_fast) {
                    const bool is_interior =
                        ...
                        src_x_base >= 0 && src_x_base + 2 < p.src_w;
                    if (is_interior) {
                        const int64_t idx00 = src_y_base * p.src_w + src_x_base;
                        const int64_t idx01 = idx00 + 1;
                        const int64_t idx02 = idx01 + 1;
                        const int64_t idx10 = idx00 + p.src_w;
                        const int64_t idx11 = idx10 + 1;
                        const int64_t idx12 = idx11 + 1;
                        const int64_t idx20 = idx10 + p.src_w;
                        const int64_t idx21 = idx20 + 1;
                        const int64_t idx22 = idx21 + 1;

                        float sum = bias_val;
                        sum += knl_data[0] * src_data[idx00];
                        sum += knl_data[1] * src_data[idx01];
                        sum += knl_data[2] * src_data[idx02];
                        sum += knl_data[3] * src_data[idx10];
                        sum += knl_data[4] * src_data[idx11];
                        sum += knl_data[5] * src_data[idx12];
                        sum += knl_data[6] * src_data[idx20];
                        sum += knl_data[7] * src_data[idx21];
                        sum += knl_data[8] * src_data[idx22];
...
void ggml_compute_forward_conv_2d_dw(
    ...
    p.bias = dst->src[2];
    p.bias_layout = ggml_conv_bias_detect_layout(p.bias, p.channels, p.batch);
    p.algo = ggml_conv2d_dw_select_algo(p.knl_w, p.knl_h, p.dilation_x, p.dilation_y);
