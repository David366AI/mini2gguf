...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "x/y",
    "x%y",
    "gather_elements(x, indices, axis)",
    "x^2",
    "√x",
    "log(x)",
    "sin(x)",
...
static struct ggml_tensor * ggml_mod_impl(
    ...
    GGML_ASSERT(
        a->type == GGML_TYPE_I8  ||
        a->type == GGML_TYPE_I16 ||
        a->type == GGML_TYPE_I32 ||
        a->type == GGML_TYPE_I64);
    GGML_ASSERT(ggml_can_repeat(b, a));
...
struct ggml_tensor * ggml_mod(
        ...
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    return ggml_mod_impl(ctx, a, b, false);
}
...
struct ggml_tensor * ggml_mod_inplace(
        ...
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    return ggml_mod_impl(ctx, a, b, true);
}

// ggml_gather_elements

struct ggml_tensor * ggml_gather_elements(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
    ...
    GGML_ASSERT(ggml_n_dims(a) == ggml_n_dims(indices));

    for (int i = 0; i < GGML_MAX_DIMS; ++i) {
        if (i == axis) {
            continue;
        }
        GGML_ASSERT(a->ne[i] == indices->ne[i]);
    }

    struct ggml_tensor * result = ggml_new_tensor(ctx, a->type, GGML_MAX_DIMS, indices->ne);

    result->op     = GGML_OP_GATHER_ELEMENTS;
    result->src[0] = a;
...
struct ggml_tensor * ggml_conv_2d_dw_direct_bias(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        ...
        int                   dilation0,
        int                   dilation1) {
    struct ggml_tensor * result = ggml_conv_2d_dw_direct(ctx, a, b, stride0, stride1, pad0, pad1, dilation0, dilation1);
...
struct ggml_tensor * ggml_conv_2d_direct_bias(
        ...
        struct ggml_tensor  * a,     // convolution kernel [KW, KH, IC, OC]
        struct ggml_tensor  * b,     // input data [W, H, C, N]
        struct ggml_tensor  * bias,  // channel bias
        int                   s0,    // stride dimension 0
        int                   s1,    // stride dimension 1
        int                   p0,    // padding dimension 0
        int                   p1,    // padding dimension 1
