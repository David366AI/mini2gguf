...
static  __global__ void im2col_kernel(
    ...
    for (int64_t iz = blockIdx.z; iz < N_OH; iz+=MAX_GRIDDIM_Z) {
        ...
        if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
            dst[offset_dst] = (TDst) 0.0f;
        } else {
            const int64_t offset_src = iic * IC_IH_IW + in * IH_IW;
            dst[offset_dst] = (TDst) x[offset_src + iih * IW + iiw];
        }
    }
...
template <typename TSrc, typename TDst>
static void im2col_cuda(const TSrc * x, TDst * dst,
    ...
    im2col_kernel<TSrc, TDst><<<block_nums, MIN(IC_KH_KW, CUDA_IM2COL_BLOCK_SIZE) , 0, stream>>>(x, dst, IC, IW, IH, OH, OW, KW, KH,
...
void ggml_cuda_op_im2col(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int64_t type_size = ggml_type_size(src1->type);
    const int64_t IC_IH_IW = src1->nb[is_2D ? 2 : 1] / type_size;
    const int64_t N        = src1->ne[is_2D ? 3 : 2];
    const int64_t IH_IW    = src1->nb[is_2D ? 3 : 2] / type_size;

    if (src1->type == GGML_TYPE_F32) {
        const float * src1_d = (const float *) src1->data;
        if (dst->type == GGML_TYPE_F16) {
            im2col_cuda<float, half>(src1_d, (half *) dst_d, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else {
            im2col_cuda<float, float>(src1_d, (float *) dst_d, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        }
    } else {
        const half * src1_d = (const half *) src1->data;
        if (dst->type == GGML_TYPE_F16) {
            im2col_cuda<half, half>(src1_d, (half *) dst_d, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        } else {
            im2col_cuda<half, float>(src1_d, (float *) dst_d, IW, IH, OW, OH, KW, KH, IC, N, IC_IH_IW, IH_IW, s0, s1, p0, p1, d0, d1, stream);
        }
    }
}
...
static  __global__ void im2col_3d_kernel(
    ...
    for (int64_t iz = blockIdx.z; iz < N_OD_OH; iz+=MAX_GRIDDIM_Z) {
        ...
        if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW || iid < 0 || iid >= ID) {
            ...
            dst[offset_dst] = (TDst) src[offset_src];
        }
    }
}
...
// [N*IC, ID, IH, IW] => [N*OD, OH, OW, IC * KD * KH * KW]
template <typename TSrc, typename TDst>
static void im2col_3d_cuda(const TSrc * src, TDst * dst,
    int64_t N, int64_t IC, int64_t ID, int64_t IH, int64_t IW, int64_t OC,
    int64_t KD, int64_t KH, int64_t KW, int64_t OD, int64_t OH, int64_t OW,
    int64_t stride_q, int64_t stride_z, int64_t stride_y, int64_t stride_x,
    ...
    im2col_3d_kernel<TSrc, TDst><<<block_nums, MIN(IC_KD_KH_KW, CUDA_IM2COL_BLOCK_SIZE) , 0, stream>>>(src, dst, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
...
void ggml_cuda_op_im2col_3d(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src1->type == GGML_TYPE_F32) {
        const float * src1_d = (const float *) src1->data;
        if (dst->type == GGML_TYPE_F16) {
            im2col_3d_cuda<float, half>(src1_d, (half *) dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
                                        stride_q, stride_z, stride_y, stride_x,
                                        s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
        } else {
            im2col_3d_cuda<float, float>(src1_d, (float *) dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
                                         stride_q, stride_z, stride_y, stride_x,
                                         s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
        }
    } else {
        const half * src1_d = (const half *) src1->data;
        if (dst->type == GGML_TYPE_F16) {
            im2col_3d_cuda<half, half>(src1_d, (half *) dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
                                       stride_q, stride_z, stride_y, stride_x,
                                       s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
        } else {
            im2col_3d_cuda<half, float>(src1_d, (float *) dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
                                        stride_q, stride_z, stride_y, stride_x,
                                        s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
        }
    }
