...
template <typename T>
static __global__ void k_repeat(
    ...
    const int64_t src_idx = i00*s00 + i01*s01 + i02*s02 + i03*s03;
    const int64_t dst_idx = i0*s0 + i1*s1 + i2*s2 + i3*s3;
...
template <typename T>
static void repeat_cuda(const ggml_tensor * src, ggml_tensor * dst, cudaStream_t stream) {
    ...
    GGML_ASSERT(dst->nb[0] % sizeof(T) == 0);
    GGML_ASSERT(dst->nb[1] % sizeof(T) == 0);
    GGML_ASSERT(dst->nb[2] % sizeof(T) == 0);
    GGML_ASSERT(dst->nb[3] % sizeof(T) == 0);
    GGML_ASSERT(src->nb[0] % sizeof(T) == 0);
    GGML_ASSERT(src->nb[1] % sizeof(T) == 0);
    GGML_ASSERT(src->nb[2] % sizeof(T) == 0);
    GGML_ASSERT(src->nb[3] % sizeof(T) == 0);

    const size_t s0  = dst->nb[0] / sizeof(T);
    const size_t s1  = dst->nb[1] / sizeof(T);
    const size_t s2  = dst->nb[2] / sizeof(T);
    const size_t s3  = dst->nb[3] / sizeof(T);
    const size_t s00 = src->nb[0] / sizeof(T);
    const size_t s01 = src->nb[1] / sizeof(T);
    const size_t s02 = src->nb[2] / sizeof(T);
    const size_t s03 = src->nb[3] / sizeof(T);

    const int64_t ne = ggml_nelements(dst);
    const int block_size = 256;
    const int64_t block_num = (ne + block_size - 1) / block_size;

    k_repeat<T><<<block_num, block_size, 0, stream>>>(
        (const T *) src->data, (T *) dst->data,
        ne0, ne1, ne2, ne3,
        ne00, ne01, ne02, ne03,
        s0, s1, s2, s3,
        s00, s01, s02, s03);
...
void ggml_cuda_op_repeat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    if (dst->type == GGML_TYPE_I32 && dst->src[0] && dst->src[0]->type == GGML_TYPE_I32) {
        repeat_cuda<int32_t>(dst->src[0], dst, ctx.stream());
        return;
    }
...
template <typename T>
static __global__ void k_mod_same_shape(const T * src0, const T * src1, T * dst, int64_t n) {
...
template <typename T>
static __global__ void k_mod_scalar(const T * src0, T scalar, T * dst, int64_t n) {
    const int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        dst[i] = src0[i] % scalar;
    }
...
template <typename T>
static void ggml_cuda_op_mod_impl(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (ggml_are_same_shape(src0, src1)) {
        k_mod_same_shape<T><<<grid_size, block_size, 0, ctx.stream()>>>(x, y, d, n);
        return;
    }

    if (ggml_nelements(src1) == 1) {
        T scalar;
        CUDA_CHECK(cudaMemcpyAsync(&scalar, y, sizeof(T), cudaMemcpyDeviceToHost, ctx.stream()));
        CUDA_CHECK(cudaStreamSynchronize(ctx.stream()));
        k_mod_scalar<T><<<grid_size, block_size, 0, ctx.stream()>>>(x, scalar, d, n);
        return;
    }
...
void ggml_cuda_op_mod(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_I8:
            ggml_cuda_op_mod_impl<int8_t>(ctx, dst);
            break;
        case GGML_TYPE_I16:
            ggml_cuda_op_mod_impl<int16_t>(ctx, dst);
            break;
        case GGML_TYPE_I32:
            ggml_cuda_op_mod_impl<int32_t>(ctx, dst);
            break;
        case GGML_TYPE_I64:
            ggml_cuda_op_mod_impl<int64_t>(ctx, dst);
            break;
        default:
            GGML_ABORT("%s: unsupported mod type: %s", __func__, ggml_type_name(src0->type));
    }
}
