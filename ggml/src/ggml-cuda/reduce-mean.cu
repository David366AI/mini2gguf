#include "reduce-mean.cuh"

#include <algorithm>
#include <cstdint>

template <typename T>
static __device__ __forceinline__ float reduce_mean_to_float(T v);

template <>
__device__ __forceinline__ float reduce_mean_to_float<float>(float v) {
    return v;
}

template <>
__device__ __forceinline__ float reduce_mean_to_float<half>(half v) {
    return __half2float(v);
}

template <typename T>
static __device__ __forceinline__ T reduce_mean_from_float(float v);

template <>
__device__ __forceinline__ float reduce_mean_from_float<float>(float v) {
    return v;
}

template <>
__device__ __forceinline__ half reduce_mean_from_float<half>(float v) {
    return __float2half(v);
}

template <typename T>
static __global__ void reduce_mean_axis_kernel(
    ...
    for (int64_t r = threadIdx.x; r < reduce_ne; r += blockDim.x) {
        sum += reduce_mean_to_float<T>(src[src_base + r * src_axis_stride]);
    }
    ...
    if (n_warps > 1) {
        ...
        __shared__ float shared_sum[max_warps];
        ...
        if (warp_id == 0) {
            sum = lane_id < n_warps ? shared_sum[lane_id] : 0.0f;
            for (int offset = WARP_SIZE/2; offset > 0; offset >>= 1) {
                sum += __shfl_xor_sync(0xFFFFFFFF, sum, offset, WARP_SIZE);
            }
        }
    ...
    if (warp_id == 0 && lane_id == 0) {
        const int64_t dst_index =
            ...
            i2 * dst_nb2 +
            i3 * dst_nb3;
        dst[dst_index] = reduce_mean_from_float<T>(sum / (float) reduce_ne);
    }
}

template <typename T>
static void ggml_cuda_op_reduce_mean_impl(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const dim3 blocks_dim((uint32_t) num_threads, 1, 1);

    reduce_mean_axis_kernel<T><<<blocks_num, blocks_dim, 0, ctx.stream()>>>(
        src_d, dst_d,
...
void ggml_cuda_op_reduce_mean(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            ggml_cuda_op_reduce_mean_impl<float>(ctx, dst);
            break;
        case GGML_TYPE_F16:
            ggml_cuda_op_reduce_mean_impl<half>(ctx, dst);
            break;
        default:
