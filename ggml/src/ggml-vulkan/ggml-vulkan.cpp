...
struct vk_device_struct {
    ...
    // [itype 0=i8,1=i16,2=i32,3=i64][0=repeat,1=norepeat]
    vk_pipeline pipeline_mod_i[4][2];
    // [data 0=f32,1=f16][index 0=i32,1=i64]
    vk_pipeline pipeline_gather_elements[2][2];
    ...
    vk_pipeline pipeline_im2col_f16, pipeline_im2col_f16_f16;
    vk_pipeline pipeline_im2col_3d_f32, pipeline_im2col_3d_f32_f16;
    vk_pipeline pipeline_im2col_3d_f16, pipeline_im2col_3d_f16_f16;
...
struct vk_op_reduce_max_push_constants {
    uint32_t reduce_ne;
    uint32_t axis;
...
static vk_op_reduce_max_push_constants vk_op_reduce_max_push_constants_init(const ggml_tensor * src, const ggml_tensor * dst, int32_t axis) {
    const uint32_t type_size = (uint32_t) ggml_type_size(src->type);

    vk_op_reduce_max_push_constants p = {};
    p.reduce_ne = (uint32_t) src->ne[axis];
    p.axis = (uint32_t) axis;

    p.ne0 = (uint32_t) src->ne[0];
    p.ne1 = (uint32_t) src->ne[1];
    p.ne2 = (uint32_t) src->ne[2];
    p.ne3 = (uint32_t) src->ne[3];
    p.nb0 = (uint32_t) (src->nb[0] / type_size);
    p.nb1 = (uint32_t) (src->nb[1] / type_size);
    p.nb2 = (uint32_t) (src->nb[2] / type_size);
    p.nb3 = (uint32_t) (src->nb[3] / type_size);

    p.dne0 = (uint32_t) dst->ne[0];
    p.dne1 = (uint32_t) dst->ne[1];
    p.dne2 = (uint32_t) dst->ne[2];
    p.dne3 = (uint32_t) dst->ne[3];
    p.dnb0 = (uint32_t) (dst->nb[0] / type_size);
    p.dnb1 = (uint32_t) (dst->nb[1] / type_size);
    p.dnb2 = (uint32_t) (dst->nb[2] / type_size);
    p.dnb3 = (uint32_t) (dst->nb[3] / type_size);
...
#undef CREATE_BINARY
    ...
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[0][0], "mod_i8",  mod_i8_len,  mod_i8_data,  "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {0}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[0][1], "mod_i8_norepeat",  mod_i8_len,  mod_i8_data,  "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[1][0], "mod_i16", mod_i16_len, mod_i16_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {0}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[1][1], "mod_i16_norepeat", mod_i16_len, mod_i16_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[2][0], "mod_i32", mod_i32_len, mod_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {0}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[2][1], "mod_i32_norepeat", mod_i32_len, mod_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[3][0], "mod_i64", mod_i64_len, mod_i64_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {0}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_mod_i[3][1], "mod_i64_norepeat", mod_i64_len, mod_i64_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_gather_elements[0][0], "gather_elements_f32_i32", gather_elements_f32_i32_len, gather_elements_f32_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_gather_elements[1][0], "gather_elements_f16_i32", gather_elements_f16_i32_len, gather_elements_f16_i32_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_gather_elements[0][1], "gather_elements_f32_i64", gather_elements_f32_i64_len, gather_elements_f32_i64_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
    ggml_vk_create_pipeline2(device, device->pipeline_gather_elements[1][1], "gather_elements_f16_i64", gather_elements_f16_i64_len, gather_elements_f16_i64_data, "main", 3, sizeof(vk_op_binary_push_constants), {512, 1, 1}, {1}, 1);
...
#define IM2COL(bda) \
    ...
    ggml_vk_create_pipeline(device, device->pipeline_im2col_f16, "im2col_f16", im2col_f16 ## bda ## _len, im2col_f16 ## bda ## _data, "main", 2, sizeof(vk_op_im2col_push_constants), {512, 1, 1}, { device->subgroup_size }, 1, true);   \
    ...
    ggml_vk_create_pipeline(device, device->pipeline_im2col_3d_f16, "im2col_3d_f16", im2col_3d_f16 ## bda ## _len, im2col_3d_f16 ## bda ## _data, "main", 2, sizeof(vk_op_im2col_3d_push_constants), {512, 1, 1}, { 512 }, 1, true);      \
    if (device->float_controls_rte_fp16) {  \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_im2col_f16_f16, "im2col_f16_f16", im2col_f16_f16_rte ## bda ## _len, im2col_f16_f16_rte ## bda ## _data, "main", 2, sizeof(vk_op_im2col_push_constants), {512, 1, 1}, { device->subgroup_size }, 1, true);   \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_im2col_3d_f16_f16, "im2col_3d_f16_f16", im2col_3d_f16_f16_rte ## bda ## _len, im2col_3d_f16_f16_rte ## bda ## _data, "main", 2, sizeof(vk_op_im2col_3d_push_constants), {512, 1, 1}, { 512 }, 1, true);      \
    } else {    \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_im2col_f16_f16, "im2col_f16_f16", im2col_f16_f16 ## bda ## _len, im2col_f16_f16 ## bda ## _data, "main", 2, sizeof(vk_op_im2col_push_constants), {512, 1, 1}, { device->subgroup_size }, 1, true);   \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_im2col_3d_f16_f16, "im2col_3d_f16_f16", im2col_3d_f16_f16 ## bda ## _len, im2col_3d_f16_f16 ## bda ## _data, "main", 2, sizeof(vk_op_im2col_3d_push_constants), {512, 1, 1}, { 512 }, 1, true);      \
    }
...
// MatMul shaders assume dense row-major matrix tiles; views/permutations can satisfy dim01 checks
// while still carrying incompatible strides/layout for these kernels.
static bool ggml_vk_mul_mat_needs_contiguous(const ggml_tensor * tensor) {
    if (!ggml_vk_dim01_contiguous(tensor)) {
        return true;
    }
    ...
    if (ggml_is_transposed(tensor) || !ggml_is_contiguous_rows(tensor)) {
        return true;
    }
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool disable_split_k) {
    ...
    const bool x_non_contig = (ctx->device->coopmat2 && src0->type == GGML_TYPE_F32) ||
                              ggml_vk_mul_mat_needs_contiguous(src0);
    const bool y_non_contig = (ctx->device->coopmat2 && src1->type == GGML_TYPE_F32) ||
                              (src0->type == GGML_TYPE_BF16 && src1->type != GGML_TYPE_BF16) ||
                              ggml_vk_mul_mat_needs_contiguous(src1);
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    const bool x_non_contig = ggml_vk_mul_mat_needs_contiguous(src0);
    const bool y_non_contig = ggml_vk_mul_mat_needs_contiguous(src1);
...
static vk_pipeline ggml_vk_op_get_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * dst, ggml_op op) {
    ...
    case GGML_OP_MOD:
        ...
        }
        {
            int itype = -1;
            switch (src0->type) {
                case GGML_TYPE_I8:  itype = 0; break;
                case GGML_TYPE_I16: itype = 1; break;
                case GGML_TYPE_I32: itype = 2; break;
                case GGML_TYPE_I64: itype = 3; break;
                default: return nullptr;
            }
            const bool is_norepeat = ggml_are_same_shape(src0, src1);
            return ctx->device->pipeline_mod_i[itype][is_norepeat ? 1 : 0];
        }
    case GGML_OP_GATHER_ELEMENTS:
        if ((src0->type != GGML_TYPE_F32 && src0->type != GGML_TYPE_F16) ||
            (src1->type != GGML_TYPE_I32 && src1->type != GGML_TYPE_I64) ||
            src0->type != dst->type) {
            return nullptr;
        }
        return ctx->device->pipeline_gather_elements[src0->type == GGML_TYPE_F16][src1->type == GGML_TYPE_I64];
    case GGML_OP_ADD_ID:
    ...
    case GGML_OP_IM2COL:
        ...
        if (src1->type == GGML_TYPE_F16 && dst->type == GGML_TYPE_F32) {
            return ctx->device->pipeline_im2col_f16;
        }
        if (src1->type == GGML_TYPE_F16 && dst->type == GGML_TYPE_F16) {
            return ctx->device->pipeline_im2col_f16_f16;
        }
        return nullptr;
    case GGML_OP_IM2COL_3D:
        ...
        if (src1->type == GGML_TYPE_F16 && dst->type == GGML_TYPE_F32) {
            return ctx->device->pipeline_im2col_3d_f16;
        }
        if (src1->type == GGML_TYPE_F16 && dst->type == GGML_TYPE_F16) {
            return ctx->device->pipeline_im2col_3d_f16_f16;
        }
        return nullptr;
...
template <> void init_pushconst_tensor_offsets(ggml_backend_vk_context * ctx, vk_op_reduce_max_push_constants &p, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst) {
    const uint32_t a_offset = get_misalign_bytes(ctx, src0) / ggml_type_size(src0->type);
    const uint32_t d_offset = get_misalign_bytes(ctx, dst) / ggml_type_size(dst->type);
...
static void ggml_vk_mod(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    const uint32_t src0_type_size = ggml_type_size(src0->type);
    const uint32_t src1_type_size = ggml_type_size(src1->type);
    const uint32_t dst_type_size = ggml_type_size(dst->type);

    ggml_vk_op_f32<vk_op_binary_push_constants>(ctx, subctx, src0, src1, nullptr, nullptr, dst, GGML_OP_MOD, {
        (uint32_t)ggml_nelements(src0),
        (uint32_t)src0->ne[0], (uint32_t)src0->ne[1], (uint32_t)src0->ne[2],(uint32_t)src0->ne[3], (uint32_t)src0->nb[0] / src0_type_size, (uint32_t)src0->nb[1] / src0_type_size, (uint32_t)src0->nb[2] / src0_type_size, (uint32_t)src0->nb[3] / src0_type_size,
        (uint32_t)src1->ne[0], (uint32_t)src1->ne[1], (uint32_t)src1->ne[2],(uint32_t)src1->ne[3], (uint32_t)src1->nb[0] / src1_type_size, (uint32_t)src1->nb[1] / src1_type_size, (uint32_t)src1->nb[2] / src1_type_size, (uint32_t)src1->nb[3] / src1_type_size,
        (uint32_t) dst->ne[0], (uint32_t) dst->ne[1], (uint32_t) dst->ne[2],(uint32_t) dst->ne[3], (uint32_t) dst->nb[0] /  dst_type_size, (uint32_t) dst->nb[1] /  dst_type_size, (uint32_t) dst->nb[2] /  dst_type_size, (uint32_t) dst->nb[3] /  dst_type_size,
...
static void ggml_vk_gather_elements(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    const uint32_t src0_type_size = ggml_type_size(src0->type);
    const uint32_t src1_type_size = ggml_type_size(src1->type);
    const uint32_t dst_type_size = ggml_type_size(dst->type);
    const int32_t axis = ggml_get_op_params_i32(dst, 0);

    ggml_vk_op_f32<vk_op_binary_push_constants>(ctx, subctx, src0, src1, nullptr, nullptr, dst, GGML_OP_GATHER_ELEMENTS, {
...
static void ggml_vk_reduce_max(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, ggml_tensor * dst) {
    const int32_t axis = ggml_get_op_params_i32(dst, 0);
...
static void ggml_vk_im2col(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    const uint32_t src1_type_size = ggml_type_size(src1->type);
    const uint32_t offset_delta = src1->nb[is_2D ? 2 : 1] / src1_type_size;
    const uint32_t batch_offset = src1->nb[is_2D ? 3 : 2] / src1_type_size;
...
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int node_idx, ggml_tensor *node_begin, int node_idx_begin, bool last_node, bool almost_ready, bool submit){
    ...
    case GGML_OP_GET_ROWS:
        ...
        break;
    case GGML_OP_GATHER_ELEMENTS:
        ggml_vk_gather_elements(ctx, compute_ctx, src0, src1, node);
    ...
    case GGML_OP_DIV:
        ...
        break;
    case GGML_OP_MOD:
        ggml_vk_mod(ctx, compute_ctx, src0, src1, node);
        ...
        case GGML_OP_MOD:
            return (op->src[0]->type == op->src[1]->type) &&
                   ...
                   (op->src[0]->type == GGML_TYPE_I8  ||
                    op->src[0]->type == GGML_TYPE_I16 ||
                    op->src[0]->type == GGML_TYPE_I32 ||
                    op->src[0]->type == GGML_TYPE_I64);
        case GGML_OP_GATHER_ELEMENTS:
            {
                ...
                if (!ggml_are_same_shape(op, op->src[1])) {
                    return false;
                }
...
static void ggml_vk_check_results_0(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
    ...
    for (int f = 0; f < ctx->num_additional_fused_ops + 1; ++f) {
        ...
        } else if (tensor->op == GGML_OP_MOD) {
            tensor_clone = ggml_mod(ggml_ctx, src_clone[0], src_clone[1]);
        ...
        } else if (tensor->op == GGML_OP_GATHER_ELEMENTS) {
            tensor_clone = ggml_gather_elements(ggml_ctx, src_clone[0], src_clone[1], ggml_get_op_params_i32(tensor, 0));
