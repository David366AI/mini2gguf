...
void process_shaders() {
    ...
    string_to_spv("mod_i8",  "mod_int.comp", {{"A_TYPE", "int8_t"},  {"B_TYPE", "int8_t"},  {"D_TYPE", "int8_t"}});
    string_to_spv("mod_i16", "mod_int.comp", {{"A_TYPE", "int16_t"}, {"B_TYPE", "int16_t"}, {"D_TYPE", "int16_t"}});
    string_to_spv("mod_i32", "mod_int.comp", {{"A_TYPE", "int32_t"}, {"B_TYPE", "int32_t"}, {"D_TYPE", "int32_t"}});
    string_to_spv("mod_i64", "mod_int.comp", {{"A_TYPE", "int64_t"}, {"B_TYPE", "int64_t"}, {"D_TYPE", "int64_t"}});
    string_to_spv("gather_elements_f32_i32", "gather_elements.comp", {{"A_TYPE", "float"}, {"B_TYPE", "int32_t"}, {"D_TYPE", "float"}});
    string_to_spv("gather_elements_f16_i32", "gather_elements.comp", {{"A_TYPE", "float16_t"}, {"B_TYPE", "int32_t"}, {"D_TYPE", "float16_t"}, {"OPTIMIZATION_ERROR_WORKAROUND", "1"}});
    string_to_spv("gather_elements_f32_i64", "gather_elements.comp", {{"A_TYPE", "float"}, {"B_TYPE", "int64_t"}, {"D_TYPE", "float"}});
    ...
    for (std::string dim_str : {"", "_3d"}) {
        for (bool bda : {false, true}) {
            ...
            string_to_spv("im2col" + dim_str + "_f16" + bda_str, "im2col" + dim_str + ".comp", merge_maps(base_dict, {{"A_TYPE", "float16_t"}, {"D_TYPE", "float"}, {"D_SIZE", "4"}, {"BDA", bda_def}}));
            string_to_spv("im2col" + dim_str + "_f16_f16" + bda_str, "im2col" + dim_str + ".comp", merge_maps(base_dict, {{"A_TYPE", "float16_t"}, {"D_TYPE", "float16_t"}, {"D_SIZE", "2"}, {"BDA", bda_def}}));
            string_to_spv("im2col" + dim_str + "_f16_f16_rte" + bda_str, "im2col" + dim_str + ".comp", merge_maps(base_dict, {{"A_TYPE", "float16_t"}, {"D_TYPE", "float16_t"}, {"D_SIZE", "2"}, {"RTE16", "1"}, {"BDA", bda_def}}));
        }
    }
